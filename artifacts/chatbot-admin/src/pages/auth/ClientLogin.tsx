...
import { useClientLogin, getGetMeQueryKey } from "@workspace/api-client-react";
...
export default function ClientLogin() {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    login.mutate({ data: values }, {
      onSuccess: () => {
        ...
        toast({
          title: "Welcome back",
          description: "Successfully logged in to client portal.",
        });
        setLocation("/client/company");
      },
      onError: (error: any) => {
        toast({
          title: "Login failed",
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_left,_var(--tw-gradient-stops))] from-primary/10 via-background to-background pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <Card className="border-border/50 shadow-2xl bg-card/80 backdrop-blur-xl">
          <CardHeader>
            ...
            <CardDescription>Sign in to manage your AI deployment.</CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...form}>
            ...
            <div className="mt-6 text-center text-sm text-muted-foreground border-t border-border pt-4">
              Admin user? <Link href="/login" className="text-primary hover:underline">Admin Login</Link>
            </div>
          </CardContent>
        </Card>
      </div>
